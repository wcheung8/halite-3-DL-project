...
class HaliteModel:
    ...
    def __init__(self, weights=None, model=None, data=None):
        if weights:
            ...
            raise Exception("Must pass weights or model.")

    @staticmethod
    def process_game_states(game_data):
        ...
        for g in game_data:
            turn_number = 0
            game_map, moves, ships, other_ships, dropoffs, other_dropoffs = g
            turn_number += 1
            for ship in list(ships.values()):
                if random.random() < .5:
                    game_states.append((game_map, moves, ships, other_ships, dropoffs,
    ...
    def train_on_files(self, replay_folder, player_name, game_data=None):
        if not game_data:
            print(f"Training on data from {replay_folder} ({len(os.listdir(replay_folder))} files)")
            game_data = parse.parse_replay_folder(replay_folder, player_name, max_files=self.MAX_FILES)

        game_states = self.process_game_states(game_data)
        data, labels = self.generate_training_data(game_states)
    ...
    def predict_move(self, ship, game_map, me, other_players, turn_number):
        ...
        opp_dropoffs = [d.position for p in other_players for d in p.get_dropoffs()] + [p.shipyard.position for p in
                                                                                        other_players]
